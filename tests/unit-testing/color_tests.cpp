...
{
    Color c(0.5, 0.3, 0.2);
    EXPECT_NEAR(c.r, 0.5, 1e-5);
    EXPECT_NEAR(c.g, 0.3, 1e-5);
    EXPECT_NEAR(c.b, 0.2, 1e-5);
}
...
SPECTRE_TEST(Color, sum_float)
{
    ...
    Color res = c+val;
    EXPECT_EQ(res.r, .65f);
    EXPECT_NEAR(res.g, .45f, 1e-5);
    EXPECT_NEAR(res.b, .35f, 1e-5);
}
...
SPECTRE_TEST(Color, sum_float_this)
{
    ...
    c += val;
    EXPECT_EQ(c.r, .65f);
    EXPECT_NEAR(c.g, .45f, 1e-5);
    EXPECT_NEAR(c.b, .35f, 1e-5);
}
...
SPECTRE_TEST(Color, div_float)
{
    ...
    float val = 0.6f;
    Color res = c/val;
    EXPECT_NEAR(res.r, 0.833333333f, 1e-5);
    EXPECT_EQ(res.g, .5f);
    EXPECT_NEAR(res.b, 0.333333333f, 1e-5);
}
...
SPECTRE_TEST(Color, div_float_this)
{
    ...
    float val = 0.6f;
    c /= val;
    EXPECT_NEAR(c.r, 0.833333333f, 1e-5);
    EXPECT_EQ(c.g, .5f);
    EXPECT_NEAR(c.b, 0.333333333f, 1e-5);
}
...
SPECTRE_TEST(Color, RGB_constructor_components_bytes)
{
    ...
    EXPECT_NEAR(c.r, .501960814f, 1e-5);
    EXPECT_EQ(c.g, 0.f);
    EXPECT_EQ(c.b, 1.f);
}
...
SPECTRE_TEST(Color, XYZ_to_sRGB)
{
    ...
    EXPECT_NEAR(res.r, 0.61799586f, 1e-5);
    EXPECT_NEAR(res.g, 0.32961869f, 1e-5);
    EXPECT_NEAR(res.b, 0.23361403f, 1e-5);

    ColorXYZ c2(0.002f, 0.002f, 0.002f);
    ColorRGB res2 = c2.to_sRGB();
    EXPECT_NEAR(res2.r, 0.031131625, 1e-5);
    EXPECT_NEAR(res2.g, 0.024504095, 1e-5);
    EXPECT_NEAR(res2.b, 0.023484498, 1e-5);
}
...
SPECTRE_TEST(Color, XYZ_to_Adobe_rgb)
{
    ...
    EXPECT_NEAR(res.r, 0.27532971f, 1e-5);
    EXPECT_NEAR(res.g, 0.48534867, 1e-5);
    EXPECT_NEAR(res.b, 0.285955f, 1e-5);
}
...
SPECTRE_TEST(Color, RGB_to_XYZ)
{
    ...
    EXPECT_NEAR(res.r, 0.15441862f, 1e-5);
    EXPECT_NEAR(res.g, 0.083299257f, 1e-5);
    EXPECT_NEAR(res.b, 0.061038386f, 1e-5);

    ColorRGB c2(0.03f, 0.03f, 0.03f);
    ColorXYZ res2 = c2.to_XYZ();
    EXPECT_NEAR(res2.r, 0.0022069735, 1e-5);
    EXPECT_NEAR(res2.g, 0.0023219816, 1e-5);
    EXPECT_NEAR(res2.b, 0.002528243, 1e-5);
}
