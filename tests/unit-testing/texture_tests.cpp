...
SPECTRE_TEST(Texture, Calculate_differentials)
{
    ...
    Ray ry;
    //nx>ny,nz
    hit.normal_g = Normal(0.7f, 0.6f, 0.5f);
    ...
    //ny>nz
    hit.normal_g = Normal(0.3f, 0.6f, 0.5f);
    calculate_differentials(&hit, &rx, &ry);
    ...
    EXPECT_TRUE(hit.differentials);
    //ny<=nz && nx<=ny && nx<=nz
    hit.normal_g = Normal(0.3f, 0.5f, 0.8f);
    calculate_differentials(&hit, &rx, &ry);
    ...
    hit.normal_g = Normal(0.249578759f, 0.70821166f, 0.150726095f);
