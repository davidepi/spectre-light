...
TEST(Box,AABB_world_space)
{
    ...
    EXPECT_TRUE(flt_equal(box.bounds[0].y, 0)); //fuck floats
    EXPECT_FLOAT_EQ(box.bounds[0].z, 0);
    EXPECT_FLOAT_EQ(box.bounds[1].x, 0);
    EXPECT_FLOAT_EQ(box.bounds[1].y, 1);
    EXPECT_FLOAT_EQ(box.bounds[1].z, 1);
...
TEST(Box,get_faces_number)
{
    Box b;
    EXPECT_EQ(b.get_faces_number(),6);
}

TEST(Box,surface_object)
{
    Box b;
    EXPECT_EQ(b.surface(), 6);
}

TEST(Box,surface_world)
{
    Box b;
    Matrix4 m;
    m.set_scale(2.5f);
    EXPECT_FLOAT_EQ(b.surface(&m), 37.5f);

    m.set_scale(Vec3(1,2,3));
    EXPECT_FLOAT_EQ(b.surface(&m), 22.f);
}
