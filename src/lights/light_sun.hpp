 ...
 *
 *  This class implements a light useful to simulate the sun. This kind of light can
 *  be seen as a spotlight infinitely far away, that does not lose power based on its
 *  distance (otherwise the power would be zero). Thus the light beam arrives in a
 *  single direction to every point of the scene.
 */
...
public:
     ...
     *
     *  Given a Date between 2000BC and 6000AD, an hour and the position of the camera,
     *  this function calculates the correct position of the sun in the sky
     *
     *  \param[in] intensity Intensity of the sun
     *  \param[in] world_centre The centre of the scene
     *  \param[in] world_rad The radius of the scene
     *  \param[in] time A Date object representing the desired instant of time
     *  \param[in] latitude The latitude (+ North, - South) of the camera position
     *  \param[in] longitude The longitude (+ East, - West) of the camera position
     *  \param[in] elevation The altitude at which the user is
     */
    LightSun(const Spectrum& intensity, const Point3* world_centre,
     ...
     *  \param[out] distance The distance of the light from the position. Since the
     *  sun is placed outside the scene, this will be always greater than two times the
     *  scene size
     *  \return The emitted radiance from the computed ray
     *  \sa sample_surface
     ...
     *  \return 1.f since only one ray is generated from the sun, always with the same
     *  pdf
     *  \sa sample_surface()
     */
     ...
     *  is infinitely small and infinitely far away, the pdf of sample_visible_surface
     *  is 0.
     *
     *  \param[in] p UNUSED
     *  \param[in] wi UNUSED
...
private:
    ...
    /**
     * A ray where origin is the sun position outside the scene
     *  and direction the sunray dir
     */
    Ray lray;
