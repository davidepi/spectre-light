                  ...
                  Sampler* sam, OcclusionTester* ot)
{
    ...
    if(hp->asset_h->is_light())
        if(dot(hp->geometric.n, -r->direction)>0)
    ...
    if(nlights>0)
    {
        ...
        {
            Spectrum bsdf_f = mat->value(&wo, hp, &wi, false);
            Ray r2(hp->point_h, wi);
            if(!bsdf_f.is_black() && !ot->is_occluded(&r2, light_distance))
            {
                bsdfpdf = mat->pdf(&wo, hp, &wi, false);
                if(bsdfpdf>0)
                {
                    float weight = (lightpdf*lightpdf)/(lightpdf*lightpdf+
                                                        bsdfpdf*bsdfpdf);
                    L += bsdf_f*direct_l*absdot(wi, hp->geometric.n)*weight/
                         lightpdf;
                }
            }
        }
        ...
        if(bsdfpdf>0 && !bsdf_f.is_black())
        {
            ...
            if(sc->k.intersect(&r2, &h2))
                if(h2.asset_h->get_id() == light->get_id())
                    if(dot(h2.geometric.n, -r2.direction)>0)
                    {
                        Spectrum rad = light->emissive_spectrum();
                        L += bsdf_f*rad*absdot(wi, hp->geometric.n)*w/bsdfpdf;
                    }
        }
    }
    return L;
