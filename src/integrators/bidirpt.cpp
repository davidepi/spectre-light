...
{
    PVertex lp[DEFAULT_BOUNCES]; //light path
    char lp_size = 0;
    PVertex ep[DEFAULT_BOUNCES]; //eye path
    char ep_size = 0;
    Color L;
    ...
    for(char i=1;i<DEFAULT_BOUNCES;i++)
    {
        ...
        BdfFlags matched;
        lp[i].l = mat->df_s(rand[1],rand[2],rand[3],&wo,&lp[i].hit,&lp[i].dir,
                                  &lp[i].pdf, BdfFlags(ALL), &matched);
    ...
    for(char i=1;i<DEFAULT_BOUNCES;i++)
    {
        ...
        BdfFlags matched;
        ep[i-1].l = mat->df_s(rand[1],rand[2],rand[3],&ep[i-1].dir,&ep[i-1].hit,
                              &ep[i].dir,&ep[i-1].pdf, BdfFlags(ALL), &matched);
    ...
    if(ep_size==DEFAULT_BOUNCES-1)
    {
        Vec3 wi;
        const Bsdf* mat = ep[ep_size].hit.hit->getMaterial();
        BdfFlags matched;
        ep[ep_size].l = mat->df_s(rand[1],rand[2],rand[3],
