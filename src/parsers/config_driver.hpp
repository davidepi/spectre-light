...
/// Type of lights that can be created
enum light_t
{
    OMNI, SPOT, AREA
};
...
///Class used to hold information about a mask, until the texture are parsed
class ParsedMask
{
public:
    ...
    ParsedMask():mask_chn(ALPHA), mask_inv(false)
    {};
};

/**
 *  \brief Class used to represent a mesh in WorldSpace.
 *  Called Asset in the renderer.
 *  This class has been merged to the lights otherwise it would be impossible to
 *  gather the object space model in area lights
 */
class MeshWorld
{
public:
...
public:
    ...
    ///If this class represents a light, the light type
    light_t type;
    ...
    ///Initialize the class with default values for lights
    ParsedLight():type(AREA), scale(1), temperature(-1), color(255, 255, 255),
                  radius(1.f), falloff(0.75f)
    {};
};

/// Struct used to hold information about a dualmaterial parsed by ConfigParser
class ParsedDualMaterial
{
public:
...
private:
    ...
    //entire lifecycle handled inside parse() function
    ...
    /**
     *  \brief Resolve the texture required by a mask
     *
     *  Transform a ParsedMask into a mask. This method should be called
     *  after parsing every texture
     *
     *  \param[in] mask The ParsedMask holding parsed information about a mask
     *  \return The newly created mask
     */
    MaskBoolean build_mask(const ParsedMask& mask);
    ...
    ///Temp val of the current dual material being parsed
    ParsedDualMaterial cur_dualmat;

    ///List of dualmaterials that will be added after adding every other mat
    std::vector<ParsedDualMaterial> deferred_dualmats;

    ///Add the materials contained in the deferred_dualmats array
    void build_dualmaterials();
    ...
    /*-----------------------------------------------.
    |          World space models and Lights         |
    `-----------------------------------------------*/

    ///Mesh in world space currently being parsed
    MeshWorld cur_mesh;

    ///List of meshes that will be added after adding every shape
    ...
    ///Light in world space currently being parsed
    ParsedLight cur_light;

    ///List of lights that will be added after adding every shape
    std::vector<ParsedLight> deferred_lights;

    ///Build lights and add them to scene
    void build_lights();
};
